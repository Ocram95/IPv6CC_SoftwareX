...
class Traffic_Class_CC:
	...
	def write_csv(self):
		...
		filename="traffic_class_cc_" + self.filepath.replace("../", "", 1) + "_number_of_packets_" + "_role_" + self.role + "_clean_packets_" + str(self.number_clean_packets) + "_number_stegopackets_" + str(self.length_stego_packets) + ".csv"
	...
	def print_start_message(self):
		...
		print('- Number of Repetitions: ' + str(self.number_of_repititions))		
	...
	def statistical_evaluation_sent_packets(self):
		...
		print("- Number of Repetitions: " + str(self.number_of_repititions_done) + "/" + str(self.number_of_repititions))
...
if __name__ == "__main__":
	...
	traffic_class_cc = Traffic_Class_CC(settings.filepath, helper.read_binary_file_and_return_chunks(settings.filepath, helper.IPv6_HEADER_FIELD_LENGTHS_IN_BITS["Traffic Class"]), settings.role, settings.consecutive_clean, settings.consecutive_stego)
