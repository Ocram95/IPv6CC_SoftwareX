...
class Hop_Limit_CC:
	...
	def __init__(self, filepath, chunks, role, number_clean_packets, length_stego_packets):
		...
		:param list chunks: A list of strings in binary format, which shall be injected into the hop limit field.
	...
	def inject(self, packet):
		...
		'''
	   	...
	   	'''
	   	# sleep necessary for HL: it can handle the big amount of packets in the queue of this cc
					...
					# If it is monotonically increasing 
						...
						if self.stegotime:
							...
							if self.chunks[self.sent_received_chunks] == '1':
								pkt.hlim += 20
								self.exfiltrated_data.append(('1', pkt.fl, pkt[TCP].seq))
							else:
								pkt.hlim -= 20
								self.exfiltrated_data.append(('0', pkt.fl, pkt[TCP].seq))
